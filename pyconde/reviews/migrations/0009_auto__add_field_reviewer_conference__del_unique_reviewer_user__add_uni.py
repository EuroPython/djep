...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Reviewer', fields ['user']
        db.delete_unique(u'reviews_reviewer', ['user_id'])

        # Adding field 'Reviewer.conference'
        db.add_column(u'reviews_reviewer', 'conference',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['conference.Conference']),
                      keep_default=False)

        # Adding unique constraint on 'Reviewer', fields ['conference', 'user']
        db.create_unique(u'reviews_reviewer', ['conference_id', 'user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Reviewer', fields ['conference', 'user']
        db.delete_unique(u'reviews_reviewer', ['conference_id', 'user_id'])

        # Deleting field 'Reviewer.conference'
        db.delete_column(u'reviews_reviewer', 'conference_id')

        # Adding unique constraint on 'Reviewer', fields ['user']
        db.create_unique(u'reviews_reviewer', ['user_id'])
    ...
    models = {
        ...
        u'reviews.reviewer': {
            'Meta': {'unique_together': "(('conference', 'user'),)", 'object_name': 'Reviewer'},
            'conference': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['conference.Conference']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
